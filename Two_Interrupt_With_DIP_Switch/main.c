...
int main(void)
{
	DIO_SetPinDirection(PORTA,PIN0,OUTPUT); /* A0 : OUTPUT (Led definition) */
	DIO_SetPinDirection(PORTA,PIN1,OUTPUT); /* A1 : OUTPUT (Led definition) */
	...
	SET_BIT(MCUCR,PIN0);
	CLR_BIT(MCUCR,PIN1); /* ExternaL Interrupt 0 happen on : Any Logical Change                 */

	SET_BIT(MCUCR,PIN2);
	CLR_BIT(MCUCR,PIN3); /* ExternaL Interrupt 0 happen on : Any Logical Change                  */
